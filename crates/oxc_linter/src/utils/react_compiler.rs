...
fn react_compiler_plugin_options() -> PluginOptions {
    PluginOptions {
        ...
        // Suppressed lines are filtered in `build_react_compiler_results`
        // instead (like the eslint plugin), so other diagnostics in the same
        // function still surface.
...
///
/// Flow-suppressed findings are already removed.
///
...
/// Run the compiler in lint mode and drop findings Flow already reported.
///
...
pub fn build_react_compiler_results(host: &ContextHost) -> ReactCompilerResults {
    ...
    let suppressed_lines = flow_suppression_lines(semantic);
    let source_text = semantic.source_text();
    let diagnostics = result
        .diagnostics
        .into_iter()
        .filter(|d| !is_flow_suppressed(&d.diagnostic, &suppressed_lines, source_text))
        .collect();
    ReactCompilerResults { diagnostics }
}
...
pub fn run_react_compiler_rule(ctx: &LintContext, category: ErrorCategory) {
    for finding in ctx.react_compiler_results().diagnostics_for(category) {
        ctx.diagnostic(finding.diagnostic.clone());
    }
}
...
fn flow_suppression_lines(semantic: &Semantic) -> Vec<u32> {
    let source_text = semantic.source_text();
    semantic
        .comments()
        .iter()
