        ...
        match self {
            ...
            Self::Right {
                ...
                if should_group {
                    ...
                    //     b || // Comment
                    //     c;
                    let should_break = f.comments().has_printed_line_comment_in_range(
...
/// Writes the operator and the break around it for one operand of a binary-like chain,
/// per `experimentalOperatorPosition`:
///
/// - `end` (default): `<space><operator><soft break>` — the operator trails the previous line
/// - `start`: `<soft break><operator><space>`, with the operand's leading own-line comments
///   hoisted above the operator. Prettier additionally emits a stray second space before the
///   previous operand's flushed trailing line comment (`prev  // comment`) — a deliberate
///   divergence (see "Known divergences").
